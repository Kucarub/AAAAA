...
public class CommonController
{
    ...
    public AjaxResult uploadFile(MultipartFile file) throws Exception
    {
        try
        {
            ...
            // 保存附件信息
            Long attachmentId = saveAttachment(file, fileName, url);
            
            AjaxResult ajax = AjaxResult.success();
            ajax.put("url", url);
            ajax.put("fileName", fileName);
            ajax.put("attachmentId", attachmentId);
    ...
    public AjaxResult uploadFiles(List<MultipartFile> files) throws Exception
    {
        try
        {
            ...
            ajax.put("attachmentIds", StringUtils.join(attachmentIds, FILE_DELIMITER));
     ...
     * 保存附件信息到数据库
     */
    private Long saveAttachment(MultipartFile file, String fileName, String url)
    {
        Attachment attachment = new Attachment();
        attachment.setOriginalName(file.getOriginalFilename());
        attachment.setStoreName(FileUtils.getName(fileName));
        attachment.setStorePath(fileName);
        attachment.setFullUrl(url);
        attachment.setFileSize(file.getSize());
        attachment.setFileType(file.getContentType());
        attachment.setSuffix(FileUploadUtils.getExtension(file));
        try {
            attachment.setUploadBy(SecurityUtils.getUserId());
        } catch (Exception e) {
            log.warn("获取上传人ID失败，可能为匿名上传: {}", e.getMessage());
        }
